...
class GrainAnalyzer:
    ...
    def calculate_grain_properties(self, min_area=50):
        """
        ...
        # Filter out small regions (likely artifacts)
        filtered_properties = [prop for prop in properties if prop.area >= min_area]
        ...
        print(f"Properties calculated for {len(self.grain_properties)} grains")
    ...
    def visualize_results(self, save_path=None):
        ...
        # 3. Segmented grains
        axes[0, 2].imshow(self.labeled_grains, cmap='nipy_spectral')
        axes[0, 2].set_title(f'Segmented Grains (n={len(self.grain_properties)})')
        axes[0, 2].axis('off')
