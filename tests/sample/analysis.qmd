---
title: "C2600P 真鍮の結晶粒径解析：加工なし vs 600°C / 60分焼なまし"
lang: ja
date: today
format:
  gfm:
...
# ---- パラメータ読み込み ----
params_asis <- fromJSON("c2600p_asis_params.json")
params_heat <- fromJSON("c2600p_600c60min_params.json")

# ---- 粒径データ読み込み ----
cond_levels <- c("加工なし", "600°C/60分焼なまし")

asis <- read_csv("c2600p_asis_grain.csv", show_col_types = FALSE) |>
  mutate(condition = factor("加工なし", levels = cond_levels))

heat <- read_csv("c2600p_600c60min_grain.csv", show_col_types = FALSE) |>
  mutate(condition = factor("600°C/60分焼なまし", levels = cond_levels))
...
n_heat  <- stats_tbl |> filter(condition == "600°C/60分焼なまし") |> pull(粒数)
mu_asis <- stats_tbl |> filter(condition == "加工なし")         |> pull(平均径_µm)
mu_heat <- stats_tbl |> filter(condition == "600°C/60分焼なまし") |> pull(平均径_µm)
med_asis <- stats_tbl |> filter(condition == "加工なし")        |> pull(中央値径_µm)
med_heat <- stats_tbl |> filter(condition == "600°C/60分焼なまし")|> pull(中央値径_µm)
sd_asis <- stats_tbl |> filter(condition == "加工なし")         |> pull(標準偏差_µm)
sd_heat <- stats_tbl |> filter(condition == "600°C/60分焼なまし") |> pull(標準偏差_µm)
...
fit_info <- bind_rows(
  ...
  extract_fit_info(fits_heat, "600°C/60分焼なまし")
)

# 各条件でAICが最小の分布名を取得
best_asis <- fit_info |> filter(条件 == "加工なし")         |> slice_min(AIC, n = 1) |> pull(分布)
best_heat <- fit_info |> filter(条件 == "600°C/60分焼なまし") |> slice_min(AIC, n = 1) |> pull(分布)
...
C2600P は Cu 70%・Zn 30% の黄銅合金板であり，単相 α（FCC）組織を示す。
本文書では grainsize\_measure ツールで解析した二つの試料を比較する。

- **加工なし**（c2600p\_asis）：未熱処理の原板
- **焼なまし**（c2600p\_600c60min）：600°C・60分焼なまし後に空冷
...
焼なまし処理によって，結晶粒径は顕著に増大した。

- **粒数**：加工なし `r n_asis` 粒 → 焼なまし `r n_heat` 粒（約 `r round(100*(1 - n_heat/n_asis))`% 減少）
- **平均等価円直径**：加工なし `r round(mu_asis, 1)` µm → 焼なまし `r round(mu_heat, 1)` µm（約 `r round(mu_heat/mu_asis, 1)` 倍）
- **中央値**：加工なし `r round(med_asis, 1)` µm → 焼なまし `r round(med_heat, 1)` µm
- **標準偏差**：加工なし `r round(sd_asis, 1)` µm → 焼なまし `r round(sd_heat, 1)` µm

粒径分布は両条件とも右裾の長い非対称分布を示した。
AIC 最小の最適モデルは，加工なしが **`r best_asis`**，焼なましが **`r best_heat`** であった。

Hall-Petch 則によれば，粒径の増大は降伏応力の低下（軟化）をもたらし，
延性・加工性の向上が期待される。
600°C・60分焼なまし材は，プレス成形など塑性加工前の中間焼なましとして
適切な軟化状態にあると考えられる。
...
| オリジナル | 粒界オーバーレイ |
|:---:|:---:|
| ![加工なし オリジナル](c2600p_asis.png) | ![加工なし 粒界検出](c2600p_asis_overlay.png) |

### 600°C / 60分焼なまし

| オリジナル | 粒界オーバーレイ |
|:---:|:---:|
| ![焼なまし オリジナル](c2600p_600c60min.png) | ![焼なまし 粒界検出](c2600p_600c60min_overlay.png) |

加工なし材では微細な粒が密に分布しているのに対し，
焼なまし材では明らかに粒が粗大化し，粒界が明瞭に観察できる。
...
param_tbl <- tibble(
  ...
  `600°C/60分焼なまし` = as.character(unlist(params_heat[param_keys]))
)
...
- **CLAHE クリップ上限**：焼なまし材ではコントラスト強調（clip = 5.0）を適用。粒界コントラストが低い大粒に対応。
- **モルフォロジー閉演算半径**：焼なまし材で大きく設定（1 → 3 px）し，太くなった粒界を確実に閉じる。
- **最小粒面積 / 最小特徴サイズ**：粒が粗大化しているため，ノイズ除去の閾値を引き上げ（9 → 50 px²）。
- **適応閾値ブロックサイズ**：各画像の輝度分布に合わせて調整（23 → 15 px）。
...
stats_tbl |>
  ...
  kable(align = c("l", rep("r", 8)))
```

焼なまし材は粒数が大幅に減少（`r n_asis` → `r n_heat` 粒）し，
...
焼なまし材の分布は全体的に大径側にシフトし，裾が長い右歪み分布を示す。
...
> **注記：ベータ分布について**
> ベータ分布のサポートは閉区間 [0, 1] に限定されるため，
> µm 単位の粒径データには適用できない。本解析では
...
```{r fit-table}
kable(fit_info, align = c("l", "l", "l", "l", "r", "r"))
```
...
plot_fit_overlay(heat, fits_heat, "600°C/60分焼なまし：観測ヒストグラムと理論分布")
```
...
qqcomp(
  list(fits_asis$lnorm, fits_asis$weibull, fits_asis$gamma),
  legendtext = c("対数正規分布", "ワイブル分布", "ガンマ分布"),
  main = "Q-Qプロット：加工なし"
)
```
...
qqcomp(
  list(fits_heat$lnorm, fits_heat$weibull, fits_heat$gamma),
  legendtext = c("対数正規分布", "ワイブル分布", "ガンマ分布"),
  main = "Q-Qプロット：600°C/60分焼なまし"
)
```
